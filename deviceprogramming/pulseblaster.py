...
def program_a_dds(number, group, initialamp, initialfreq):
    ...
    amps = group['AMP_REGS'][:] # slice copies, we don't intend to modify the originals
...
if __name__== '__main__':        
    ...
    with h5py.File(sys.argv[-1],'r') as hdf5_file:
        try:
            pb_init()
            pb_core_clock(75)
            for group in hdf5_file['devices']:
                if group.startswith('pulseblaster'):
                    number = int(group.split('_')[-1])
                    final_states[number] = program_a_pulseblaster(number, hdf5_file['devices/%s'%group], *initial_states[number])
            pb_close()
        except:
            pb_close()
            raise
    for pb in final_states:
        for item in final_states[pb]:
            print item,
